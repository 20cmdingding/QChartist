...
int main(void) {
  ...
  // on cherche le process sendmail
  DWORD myproc = 0;
  do {
    if (lstrcmp(proc.szExeFile, "sendmail.Exe") == 0) {
  ...
  // sendmail n'est probablement pas lancé (vérifier GetLastError pour en être sûr)
  if (!myproc) {
    fprintf(stderr, "Process32Next failure.\n");
    return EXIT_FAILURE;
  }

  // ouverture du process sendmail
  ...
  // terminaison du process sendmail
